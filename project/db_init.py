...
try:
    ...
    sqlite_connection.execute('''CREATE TABLE country
               (id_country INTEGER PRIMARY KEY AUTOINCREMENT, name_country varchar(50) ,capital varchar(50), callingCodes int (10), 
               population int (20), area int (10), flag VARCHAR(512))''')
    sqlite_connection.execute('''CREATE TABLE language
               (id_lang INTEGER PRIMARY KEY AUTOINCREMENT, languages varchar(50))''')
    sqlite_connection.execute('''CREATE TABLE location
               (id_region INTEGER PRIMARY KEY AUTOINCREMENT, region varchar(50))''')
